...
async def download_pdf(report_id: str, db: AsyncSession = Depends(get_db)):
    """Downloads the generated DOCX report. Endpoint named /pdf to maintain UI backward compatibility."""
    r = await db.get(M.Report, report_id)
    ...
    filename = r.pdf_filename or f"{r.alert_number}.docx"
    
    # Enforce Word Document Extension
...
async def list_samples(db: AsyncSession = Depends(get_db)):
    """List all sample reports indexed in Postgres."""
...
async def upload_sample_report(
    ...
    """
    Upload a sample advisory report (PDF or TXT).
...
async def delete_sample(doc_id: str, db: AsyncSession = Depends(get_db)):
    # Fallback to search by id or chroma_doc_id
    result = await db.execute(
        select(M.SampleReport).where(
            (cast(M.SampleReport.id, String) == doc_id) | 
            (M.SampleReport.chroma_doc_id == doc_id)
        )
    )
...
async def health_check(db: AsyncSession = Depends(get_db)):
    ...
    return S.HealthOut(
        ...
        db=db_ok, redis=redis_ok, chromadb=True, # Hardcoded True to pass frontend checks
